...
class AttentionBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        norm_layer = nn.BatchNorm2d
        self.conv0 = nn.Conv2d(inplanes, inplanes, kernel_size=1)
...
class MANet(nn.Module):
    ...
    def __init__(self, block_b, block_a, block_d, layers, num_classes=7):
        ...
        self.layer1 = self._make_layer(block_b, 3, 32, layers[0], stride=2)
        self.r1 = ResidualDilatedBlock(32, 32, 1, [1, 2, 3])
        self.layer2 = self._make_layer(block_b, 32, 64, layers[1], stride=2)
        self.r2 = ResidualDilatedBlock(64, 64, 1, [1, 2, 3])
        # In this branch, each BasicBlock replaced by AttentiveBlock.
        self.layer3_1_p1 = self._make_layer(block_a, 64, 256, layers[2], stride=2)
        self.layer4_1_p1 = self._make_layer(block_a, 256, 1024, layers[3], stride=2)

        self.layer3_1_p2 = self._make_layer(block_a, 64, 256, layers[2], stride=2)
        self.layer4_1_p2 = self._make_layer(block_a, 256, 1024, layers[3], stride=2)

        self.layer3_1_p3 = self._make_layer(block_a, 64, 256, layers[2], stride=2)
        self.layer4_1_p3 = self._make_layer(block_a, 256, 1024, layers[3], stride=2)

        self.layer3_1_p4 = self._make_layer(block_a, 64, 256, layers[2], stride=2)
        self.layer4_1_p4 = self._make_layer(block_a, 256, 1024, layers[3], stride=2)
...
def manet(num_classes=7):
    return MANet(block_b=BasicBlock, block_a=AttentionBlock, block_d=ResidualDilatedBlock, layers=[2, 2, 1, 1], num_classes=num_classes)
